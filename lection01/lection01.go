...
package main
func main() {}
^^^^^^^^^^^^ всё, что выше, должно присутствовать, если это самостоятельная программа

* В директории (именно её мы импортим) может присутствовать только 1 пакет, но пакет НЕ ОБЯЗАН называться по имени
директории (хотя для определённости рекомендуется)
...
*/
func funcs() {
	// variadic function - функция с переменным числом аргументов
	// variadic аргумент - слайс
...
* panic(v) - завершает программу, принимая interface{}, который можно перехватить функцией recover
* recover() - перехватывает объект, переданный в panic и возвращает управление функции, в которой была вызвана
* defer someFunc(someArg) - кладёт в стек (локальный для каждой вызывающей функции) вызов функции, и вызывает все
функции поочерёдно перед завершением основной
* НЕ ИСПОЛЬЗОВАТЬ, т.к. отловка и вызов ошибок может говорить о том, что код ненадёжный
*/
func panicRecover() {
	defer func() {
	...
	callPanic := func() {
		defer func() {
			fmt.Println("callPanic defer")

			if recoveryMsg := recover(); recoveryMsg != nil {
				fmt.Println("recoveryMsg:", recoveryMsg)
			}
		}()

		panic("callPanic panic")
	}
...
* Выполняется при импорте пакета, т.е. перед main()
* Можно объявлять НЕСКОЛЬКО функций init в одном файле
* В основном используется для инициализации глобальных переменных, которые нельзя просто так инициализировать, и для
каких-либо одноразовых вычислений
* НЕ ИСПОЛЬЗОВАТЬ, т.к. плохой тон
* Порядок инициализации любого пакета:
1) Выполнение всех import'ов
2) Вычисление и присвоение значений глобальным переменным из ВСЕХ импортируемых файлов
3) Выполнение всех init'ов (в порядке объявления внутри каждого файла) из всех файлов в лексикографическом порядке (но
сначала файл с main())
* Инициализация пакета выполняется только ОДИН РАЗ, даже если пакет импортируется много раз
* Пустой импорт (import _ "somepkg") используют, когда требуется выполнить ТОЛЬКО init'ы из пакета. Неиспользуемые
непустые импорты сами по себе вызывают CE
*/
//var _ = initBlank()
//
//func initBlank() int {
//	fmt.Println("blank identifier initialization")
//
//	return 123
//}
//
//func init() {
//	fmt.Println("main: first init")
//}
//
//func init() {
//	fmt.Println("main: second init")
//}

func initTest() {
	// init() // CE - нельзя вызвать, т.к. вызывается автоматически только один раз, + таких init'ов мб несколько
}
