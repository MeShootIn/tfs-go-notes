...
* БД перебором выбирает план с наименьшей стоимостью (cost = a..b), где a - оценочная стоимость получения 1-ой строки,
b - последней. Стоимости формируются при помощи неточной статистики из абстрактного времени выполнения операций. Чем
сложнее схема (schema), тем выше шанс статистической ошибки, т.к. она основана на переборе всех строк таблицы (Seq Scan).
